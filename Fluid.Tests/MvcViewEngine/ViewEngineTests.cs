...
namespace Fluid.Tests.MvcViewEngine
{
    public class ViewEngineTests
    {
        ...
        {
            _options.RenderingViewAsync = (view, context) =>
            {
                context.SetValue("custom", "hello");
                return default;
            };
        ...
        [Fact] 
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_Nested_NoValueRender()
        {
            var bigString = new string(Enumerable.Range(0, 1500).Select(_ => 'b').ToArray());
        ...
        [Fact]
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_NotNested()
        {
            _mockFileProvider.Add("Views/Index.liquid", "{% layout '_Layout' %}{{BigString }}");
        ...
        [Fact]
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_Nested_SmallBuffer_128LengthString()
        {
            _mockFileProvider.Add("Views/Index.liquid", "{% layout '_Layout' %}{% section bigboy %}{{BigString}}{% endsection %} ");
            _mockFileProvider.Add("Views/_Layout.liquid", "{% rendersection bigboy %}");

            await using var sw = new StreamWriter(new NoSyncStream(), bufferSize: 10);
            var template = new TemplateContext(new {BigString = new string(Enumerable.Range(0, 128).Select(_ => 'b').ToArray())});
            await _renderer.RenderViewAsync(sw, "Index.liquid", template);
            await sw.FlushAsync();
        }
        ...
        [Fact]
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_NonNested_SmallBuffer_128LengthString()
        {
            _mockFileProvider.Add("Views/Index.liquid", "{% layout '_Layout' %}{% section bigboy %}{{BigString}}{% endsection %} ");
            _mockFileProvider.Add("Views/_Layout.liquid", "{% rendersection bigboy %}");

            await using var sw = new StreamWriter(new NoSyncStream(), bufferSize: 10);
            var template = new TemplateContext(new {BigString = new string(Enumerable.Range(0, 128).Select(x => 'b').ToArray())});
            await _renderer.RenderViewAsync(sw, "Index.liquid", template);
            await sw.FlushAsync();
        }

        [Fact]
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_NonNested_SmallBuffer_BiggerThan128LengthString()
        {
        ...
        private class NoSyncStream : Stream
        {
