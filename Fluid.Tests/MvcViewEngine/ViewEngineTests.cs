...
namespace Fluid.Tests.MvcViewEngine
{
    public class ViewEngineTests
    {
        ...
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_Nested()
        {
            ...
            var template = new TemplateContext(new {BigString = new string(Enumerable.Range(0, 1500).Select(x => 'b').ToArray())});
        ...
        [Fact]
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_Nested_SmallBuffer()
        {
            _mockFileProvider.Add("Views/Index.liquid", "{% layout '_Layout' %}{% section bigboy %}{{BigString}}{% endsection %} ");
            _mockFileProvider.Add("Views/_Layout.liquid", "{% rendersection bigboy %}");

            await using var sw = new StreamWriter(new NoSyncStream(),bufferSize:50);
            var template = new TemplateContext(new {BigString = new string(Enumerable.Range(0, 1500).Select(x => 'b').ToArray())});
            await _renderer.RenderViewAsync(sw, "Index.liquid", template);
            await sw.FlushAsync();
        }
        ...
        [Fact]
        public async Task RenderViewOnlyAsyncStream_LargePropertyValue_NotNested_SmallBuffer()
        {
            _mockFileProvider.Add("Views/Index.liquid", "{% layout '_Layout' %}{{BigString }}");
            _mockFileProvider.Add("Views/_Layout.liquid", "{% renderbody %}");
